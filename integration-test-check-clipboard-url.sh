...
if [ "$SKIP_CLIPBOARD_TESTS" != "true" ]; then
    ...
    if timeout 5 set_clipboard "$TEST_URL" 2>/dev/null; then
        READ_URL=$(timeout 5 get_clipboard)
...
if [ "$SKIP_CLIPBOARD_TESTS" != "true" ]; then
    # Clear clipboard
    timeout 5 set_clipboard "" 2>/dev/null
    
    # Run script with 1 retry and 1 second wait, expecting failure
    output=$(timeout 3 "$MAIN_SCRIPT" --retry-count 1 --wait-time 1 2>&1)
    exit_code=$?
...
if [ "$SKIP_CLIPBOARD_TESTS" != "true" ]; then
    timeout 5 set_clipboard "invalid" 2>/dev/null
...
if [ "$SKIP_CLIPBOARD_TESTS" != "true" ] && [ "$SKIP_BROWSER_TESTS" != "true" ]; then
    timeout 5 set_clipboard "https://www.example.com" 2>/dev/null
...
EOF
    ...
    if [ "$BROWSER_LAUNCHER" = "xdg-open" ]; then
        ...
        timeout 2 "$MAIN_SCRIPT" --retry-count 1 --wait-time 1 &>/dev/null
        result=$?
...
if [ "$SKIP_CLIPBOARD_TESTS" != "true" ] && [ "$SKIP_BROWSER_TESTS" != "true" ]; then
    timeout 5 set_clipboard "http://localhost:8080" 2>/dev/null
...
if [ "$SKIP_CLIPBOARD_TESTS" != "true" ] && [ "$SKIP_BROWSER_TESTS" != "true" ]; then
    timeout 5 set_clipboard "www.example.com" 2>/dev/null
...
if [ "$SKIP_CLIPBOARD_TESTS" != "true" ]; then
    ...
    START_TIME=$(date +%s)
    timeout 5 "$MAIN_SCRIPT" --retry-count 3 --wait-time 1 2>&1 > /dev/null
    END_TIME=$(date +%s)
...
if [ "$SKIP_CLIPBOARD_TESTS" != "true" ]; then
    ...
    # Run in infinite mode but timeout after 2 seconds
    output=$(timeout 2 "$MAIN_SCRIPT" --retry-count -1 --wait-time 1 2>&1)
    exit_code=$?
...
if [ "$SKIP_CLIPBOARD_TESTS" != "true" ] && [ "$SKIP_BROWSER_TESTS" != "true" ]; then
    timeout 5 set_clipboard "/tmp" 2>/dev/null
    
    # Run in local mode with 1 attempt - should accept /tmp as valid directory
    output=$(timeout 3 "$MAIN_SCRIPT" --local --retry-count 1 --wait-time 1 2>&1)
    exit_code=$?
...
if [ "$SKIP_CLIPBOARD_TESTS" != "true" ]; then
    timeout 5 set_clipboard "/nonexistent/directory/path" 2>/dev/null
    
    # Run in local mode with 1 retry - should reject invalid directory
    output=$(timeout 3 "$MAIN_SCRIPT" --local --retry-count 1 --wait-time 1 2>&1)
    exit_code=$?
...
if [ "$SKIP_CLIPBOARD_TESTS" != "true" ]; then
    ...
    output=$(timeout 3 "$MAIN_SCRIPT" -l --retry-count 1 --wait-time 1 2>&1)
    exit_code=$?
